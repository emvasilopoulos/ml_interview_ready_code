...
if __name__ == "__main__":
    ...
    mnn.visualize.save_model_graph_as_png(
        object_detection_model, image_RGB, pathlib.Path("model")
    )

    # Method 1: Using children()
    last_layer = list(object_detection_model.children())[-1]
    print("Last layer using children():", last_layer)
    # Get the type of the last layer
    print("Last layer type:", type(last_layer))
